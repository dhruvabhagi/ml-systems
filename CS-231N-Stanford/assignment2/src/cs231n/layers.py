...
def affine_forward(x, w, b):
    ...
    out = x_reshaped.dot(w) + b # (N,D) * (D, M) + M
    cache = (x, w, b)
    ###########################################################################
    return out, cache
...
def batchnorm_backward(dout, cache):
    ...
    # dout is the incoming gradient flowing from upstream (i.e dy)
    dgamma = (dout * x_hat).sum(axis=0)
    dx_hat = dout * gamma
    # gradient through σ²
    dvar = (dx_hat * (x - mean) * -0.5 * (var + eps)**(-1.5)).sum(axis=0)

    # gradient through μ (two terms: direct + via dvar)
    dmean = (dx_hat * -1 / np.sqrt(var + eps)).sum(axis=0) \
          + dvar * (-2 / N) * (x - mean).sum(axis=0)

    # gradient w.r.t. x (three terms)
...
def layernorm_backward(dout, cache):
    ...
    dx = (1 / (D * np.sqrt(var + eps))) * (
        ...
        - x_hat * (dy * x_hat).sum(axis=1, keepdims=True)
    )
    ###########################################################################
    return dx, dgamma, dbeta
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # pad the input along H and W dimensions only
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    h_start = i * stride # So i * stride converts from "output position" to "input position" in receptive field begin.
                    w_start = j * stride
                    # extract receptive field across all channels
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # gradient of loss w.r.t. bias — one scalar per filter
                    db[f] += dout[n, f, i, j]

                    # gradient of loss w.r.t. filter weights
                    dw[f] += patch * dout[n, f, i, j]

                    # gradient of loss w.r.t. padded input
                    dx_pad[n, :, h_start:h_start+HH, w_start:w_start+WW] += w[f] * dout[n, f, i, j]

    # strip the padding to get dx in original input shape
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    # reshape to (N*H*W, C) so batchnorm normalizes per channel
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # reshape to isolate groups: (N, G, C//G, H, W)
    x_group = x.reshape(N, G, C//G, H, W)

    # normalize over the group dimensions (C//G, H, W) i.e. axes 2, 3, 4
    mean = x_group.mean(axis=(2, 3, 4), keepdims=True)   # (N, G, 1, 1, 1)
    var  = x_group.var(axis=(2, 3, 4), keepdims=True)    # (N, G, 1, 1, 1)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dy = (dout * gamma).reshape(N, G, C//G, H, W)
    x_hat_group = x_hat.reshape(N, G, C//G, H, W)

    # number of elements normalized together per group
    D = (C//G) * H * W

    dx_group = (1 / (D * np.sqrt(var + eps))) * (
        D * dy
        - dy.sum(axis=(2, 3, 4), keepdims=True)
        - x_hat_group * (dy * x_hat_group).sum(axis=(2, 3, 4), keepdims=True)
    )
