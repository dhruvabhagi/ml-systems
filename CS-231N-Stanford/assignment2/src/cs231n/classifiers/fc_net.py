...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Initialize gamma, beta for batchnorm layers (all layers except last)
    ...
    def loss(self, X, y=None):
        ...
        # Last layer — affine only, no batchnorm/relu
        ...
        dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = affine_backward(dscores, last_cache)
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']

        # Backward pass — hidden layers in reverse
        for i in reversed(range(self.num_layers - 1)):
            ...
            if self.normalization == 'batchnorm':
              dout, grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = batchnorm_backward_alt(dout, bn_cache)
            elif self.normalization == 'layernorm':
