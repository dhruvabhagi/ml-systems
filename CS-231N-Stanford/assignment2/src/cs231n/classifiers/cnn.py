...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # Conv layer: (F, C, HH, WW)
        self.params['W1'] = np.random.randn(num_filters, C, filter_size, filter_size) * weight_scale
        self.params['b1'] = np.zeros(num_filters)
        
        # After conv (spatial size preserved) + 2x2 max pool with stride 2:
        # H and W are halved
        pool_out_H = H // 2
        pool_out_W = W // 2
        
        # Affine hidden layer
        self.params['W2'] = np.random.randn(num_filters * pool_out_H * pool_out_W, hidden_dim) * weight_scale
