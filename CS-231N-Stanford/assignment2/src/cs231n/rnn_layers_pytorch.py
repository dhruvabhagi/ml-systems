...
def rnn_step_backward(dnext_h, cache):
    ...
    - cache: Tuple of (x, prev_h, Wx, Wh, b, next_h) from forward pass
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    N, T, D = x.shape 
    # N is # of inputs/samples
    # T is # no. of timesteps in each sample
    # D is # of features
    H = h0.shape[1]
    ...
    for t in range(T): # for each Time Step
        x_t = x[:, t, :] #slices out one timestep at a time across all samples in the batch.
        next_h = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
        prev_h = next_h
        h[:, t, :] = next_h    # store hidden state at timestep t
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h
