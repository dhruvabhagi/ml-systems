...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    probs0 = mdp_data['transition_probs'][state, :, 0]
    probs1 = mdp_data['transition_probs'][state, :, 1]
    disc_value = mdp_data['reward'] + mdp_data.get('gamma', 0.995) * mdp_data['value']
    q0 = probs0.dot(disc_value)
    q1 = probs1.dot(disc_value)
    if q0 == q1:
        return np.random.randint(0, 2)  # return random action if tie
    return 0 if q0 > q1 else 1  # action with higher expected value
   # *** END CODE HERE ***
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # reward_counts[new_state, 0] = number of times reward was -1 when new_state was reached
    # reward_counts[new_state, 1] = total number of times new_state was reached
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    transition_counts = mdp_data['transition_counts']  # (num_states, num_states, 2)
    reward_counts = mdp_data['reward_counts']          # (num_states, 2)

    # Update transition probabilities for each (state, action) pair
    for a in range(2):
        # Sum of transitions from each state under action a: shape (num_states,)
        total = transition_counts[:, :, a].sum(axis=1)  # (num_states,)
        # Only update rows where we have observations
        visited = total > 0 # boolean mask for states that have been visited with action a, it has same shape as total, and is True for states that have been visited with action a
        # Normalize
        # For visited states, P(s'|s,a) = count(s,a,s') / total count of transitions from s under action a
        mdp_data['transition_probs'][visited, :, a] = (
            transition_counts[visited, :, a] / total[visited, np.newaxis]
        )
        # Unvisited states keep uniform distribution (already initialized)

    # Update reward estimates
    # R(s) = (number of times reward -1 was seen) / (total times s was reached)
    visited = reward_counts[:, 1] > 0 # boolean mask for states that have been visited at least once, it has same shape as reward_counts[:, 1], and is True for states that have been visited at least once
    mdp_data['reward'][visited] = (
        -reward_counts[visited, 0] / reward_counts[visited, 1]
    )
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    # *** START CODE HERE ***
    transition_probs = mdp_data['transition_probs']  # (num_states, num_states, 2)
    reward = mdp_data['reward']                      # (num_states)
    value = mdp_data['value']                        # (num_states)

    converged_in_one_iteration = True

    while True:
        # Q(s, a) = R(s) + gamma * sum_{s'} P(s'|s,a) * V(s')
        # transition_probs.transpose(2, 0, 1) reorders the axis from (states, new_states, actions) to (actions, states, new_states) i.e 2 x 163(old) x 163(new)
        # value has dimensions 163(state), dot product sums over the last axis, i.e, new_state and preserves the first and second dimensions of transition_probs,
        # thereby giving us an array of shape (2, 163). As a result, we get expected value for each action at each state
        # .T to flip it to (state, action) for q_values
        # note: we aren't applying transition_probs on reward, since reward is only a function of state and not action as given in the problem statement
        # i.e reward is R(s') and not any of R(s) or R(s, a) or R(s, a, s')
        q_values = reward[:, np.newaxis] + gamma * transition_probs.transpose(2, 0, 1).dot(value).T
        # q_values[s, a] = R(s) + gamma * sum_{s'} P(s'|s,a)*V(s')

        new_value = q_values.max(axis=1) # max across columns (i.e action 0, action 1) for each row (state)
...
def main(plot=True):
    ...
    # below two lines were commented out earlier, just putting it out for reference
