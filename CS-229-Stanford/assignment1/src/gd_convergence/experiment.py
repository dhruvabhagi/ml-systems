...
def update_theta(theta, lr):
    ...
    """
    gradiant = 2 * (theta.dot(A)) # derivate of (x.T)A(x) = is 2A(theta) when A is symmetric
    new_theta = theta - lr * gradiant # We are dealing with convex function, hence minus sign after theta
    return new_theta # returning the new theta
...
def gradient_descend(J, theta_0, lr, update_theta, epsilon=1e-50):
    ...
    theta = theta_0
    # *** START CODE HERE ***
    j_old = J(theta) # last iteration of J(theta) is captured here!
    while True:
        # newly computed theta after applying the derivative on loss function
        theta_new = update_theta(theta, lr)
        
        # Applying J function on the new value of the theta
        j_new = J(theta_new)

        # convergence condition attained
