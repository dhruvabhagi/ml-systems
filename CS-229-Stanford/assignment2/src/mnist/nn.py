...
def softmax(x):
    ...
    """
    # *** START CODE HERE ***
    x_reduced = x - np.max(x, axis=1, keepdims=True)
    x_exp = np.exp(x_reduced)
...
def forward_prop(data, one_hot_labels, params):
    ...
    z2 = np.dot(a1_sigmoid, params['W2']) + params['b2']
    a2_softmax = softmax(z2)
    loss = -np.mean(np.sum(one_hot_labels * np.log(a2_softmax), axis=1))
    return a1_sigmoid, a2_softmax, loss
    # *** END CODE HERE ***
...
def backward_prop(data, one_hot_labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    a1_sigmoid, a2_softmax, _ = forward_prop_func(data, one_hot_labels, params)
    batch_size = data.shape[0]
 
    # Output layer gradients
    dz2 = (a2_softmax - one_hot_labels)
    dW2 = np.dot(a1_sigmoid.T, dz2) / batch_size
    db2 = np.sum(dz2, axis=0) / batch_size

    # Hidden layer gradients
    da1 = np.dot(dz2, params['W2'].T)
    dz1 = da1 * a1_sigmoid * (1 - a1_sigmoid)
...
def backward_prop_regularized(data, one_hot_labels, params, forward_prop_func, reg):
    ...
    # Output layer gradients
    dz2 = (a2_softmax - one_hot_labels)
    dW2 = (np.dot(a1_sigmoid.T, dz2) / batch_size) + reg * params['W2']
    db2 = np.sum(dz2, axis=0) / batch_size

    # Hidden layer gradients
    da1 = np.dot(dz2, params['W2'].T)
    dz1 = da1 * a1_sigmoid * (1 - a1_sigmoid)
    dW1 = (np.dot(data.T, dz1) / batch_size) + reg * params['W1']
    db1 = np.sum(dz1, axis=0) / batch_size
...
def run_train_test(name, all_data, all_labels, backward_prop_func, num_epochs, plot=True):
    ...
    params_output_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), name + '_params.npz')
    np.savez(params_output_file_path, W1=params['W1'], b1=params['b1'], W2=params['W2'], b2=params['b2'])
